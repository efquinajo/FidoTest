...
class OcspResponse {
    ...
    protected OcspResult parseBasicResponse(BasicOCSPResp response) {
        ...
        for (SingleResp singleResponse : response.getResponses()) {
            CertificateStatus certificateStatus = parseCertificateStatus(singleResponse.getCertStatus());
            map.put(singleResponse.getCertID().getSerialNumber(),
                    new CertificateResult(
                            ...
                            certificateStatus == CertificateStatus.REVOKED ? ((RevokedStatus) singleResponse.getCertStatus()).getRevocationTime() : null
                    )
            );
        }
